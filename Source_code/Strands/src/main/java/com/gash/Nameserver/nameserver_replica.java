...
public class nameserver_replica
{
...
*/
    ...
    {
        nameserver_replica p1 = new nameserver_replica();
        p1.start();
    ...
    static public class DataCenterService extends DataCenterServiceGrpc.DataCenterServiceImplBase
    {
        ...
        @Override
        public void updateBackUpNS(Datacentre.UpdateMap request, StreamObserver<Datacentre.NameServerAprooved> responseObserver) {
                System.out.println(request.getFileName());
                System.out.println(request.getDataCenters());
                String filename = request.getFileName();
                String[] filename_extension = filename.split("\\+",2);
                System.out.println(filename_extension[0]);
                System.out.println(filename_extension[1]);
                mapping(filename_extension[0],Long.parseLong(filename_extension[1]),Integer.parseInt(request.getDataCenters()));
                System.out.println("Name Server Replica");
                for (Map.Entry<String,Map<Integer,String>> entry : dataMap.entrySet()) {
                            String key = entry.getKey().toString();
                            System.out.print("KEY RC2: " + key + ": ");
                            List<String> print_list = new ArrayList<>();
                        ...
                        System.out.println("size of map :"+ dataMap.size());
                 System.out.println("end of replica server printing");
            Datacentre.NameServerAprooved n = Datacentre.NameServerAprooved.newBuilder().setNsGot(true).build();
